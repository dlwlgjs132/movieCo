...
def get_current_movie():
    ...
    lotte_data = lotte.get_current_movie(latitude, longitude)
    cgv_data = cgv.get_current_movie(latitude, longitude)
...
def show_trailer():
    ...
    return jsonify(lotte.show_trailer(movie_name))
...
def get_specific_time_movie(latitude,longitude,start, end=30):
    ...
    if end == 30:
        ...
        return jsonify(lotte.get_specific_time_movie(latitude, longitude,time_list))
    ...
    lotte_data = lotte.get_specific_time_movie(latitude, longitude, time_list)
    cgv_data = cgv.get_specific_time_movie(latitude, longitude, time_list)
...
def get_movie_name_theater(movie_name, latitude, longitude):
    return jsonify(lotte.get_movie_name_theater(latitude, longitude, movie_name))
